...
def polar_layout(center, radius, n):
    ...
    for i in range(n):
        ...
        pts.append((int(cx + radius * math.cos(theta)),
...
def run_best_model(time_budget=120, fps=24, slowdown_ms=400, steps=0,
...
if __name__ == "__main__":
    ...
    run_best_model(
        time_budget=240,   # longer session so behavior is visible
        fps=16,            # not too fast
        slowdown_ms=600,   # extra delay to make it human-watchable
        steps=0,           # 0 = run until episode ends
        save_frames=False, # set True if you want PNGs for a video
    )
